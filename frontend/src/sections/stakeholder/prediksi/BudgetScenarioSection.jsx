...
function ScenarioSimulatorCard({ trends = [], settings }) {
  ...
  return (
    <div className="relative flex min-w-0 flex-col gap-4 overflow-hidden rounded-xl bg-[#004536] p-4 sm:gap-6 sm:p-6 xl:p-8">
      {/* Decorative blur orb */}
      ...
      <div className="flex min-w-0 items-start gap-2 sm:items-center sm:gap-3">
        ...
        <h3 className="break-words text-sm font-extrabold leading-5 text-white sm:text-base sm:leading-6">Simulator Skenario Dampak</h3>
      </div>

      {/* Controls + Result */}
      <div className="flex min-w-0 flex-col items-stretch gap-5 lg:flex-row lg:items-start lg:gap-12">
        {/* Sliders */}
        <div className="flex min-w-0 flex-1 flex-col gap-5 sm:gap-6">
          {/* Volume slider */}
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1 opacity-90 xs:flex-row xs:items-start xs:justify-between">
              <span className="break-words text-sm font-bold leading-5 text-white sm:text-base sm:leading-6">Volume Minyak Jelantah (KL)</span>
              <span className="shrink-0 text-sm font-extrabold leading-5 text-[#81F9C1] sm:text-base sm:leading-6">{volume.toLocaleString('id-ID')}</span>
            </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1 opacity-90 xs:flex-row xs:items-start xs:justify-between">
              <span className="break-words text-sm font-bold leading-5 text-white sm:text-base sm:leading-6">Harga Acuan (Rp/L)</span>
              <span className="shrink-0 text-sm font-extrabold leading-5 text-[#81F9C1] sm:text-base sm:leading-6">Rp{price.toLocaleString('id-ID')}</span>
            </div>
            <div className="relative">
              <input
                type="range"
        ...
        {/* Result card */}
        <div className="flex w-full min-w-0 flex-col items-center justify-center gap-2 self-auto rounded-xl border border-white/10 bg-white/5 p-4 sm:p-6 lg:w-auto lg:min-w-[200px] lg:self-stretch">
          ...
          <span className="max-w-full break-all text-center text-lg font-extrabold leading-7 text-[#C9A96E] sm:text-xl">Rp{netProfit}</span>
