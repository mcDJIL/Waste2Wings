...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const getActiveId = () => {
    if (pathname === '/community/map')       return 'peta'
    if (pathname.startsWith('/community/submissions')) return 'submissions'
    if (pathname.startsWith('/community/history')) return 'history'
    if (pathname.startsWith('/community/settings')) return 'settings'
    return 'dashboard'
  }
  ...
  return (
    <>
      ...
      <aside
        className={[
          'fixed top-0 left-0 h-full w-[280px] bg-[#051C37] z-1000 flex flex-col',
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 pt-7 pb-4">
          ...
          />
          <div>
            <p className="text-[#81F9C1] font-bold text-base leading-5">Waste2Wings</p>
        ...
        <div className="mx-6 my-4 flex items-center gap-3 p-4 rounded-xl bg-white/5 backdrop-blur-sm">
          <div className="w-10 h-10 rounded-full bg-[#81F9C1] overflow-hidden flex-shrink-0 flex items-center justify-center">
            <span className="text-[#004536] font-bold text-sm">
              {user?.name ? user.name.charAt(0).toUpperCase() : 'U'}
            </span>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-white text-sm font-bold leading-5 truncate">{user?.name || 'User'}</p>
            <p className="text-white/50 text-[10px] uppercase tracking-[-0.5px] leading-4">
              {user?.role?.toLowerCase() === 'community' ? 'COMMUNITY MEMBER' : 'MEMBER'}
            </p>
          </div>
          <button
