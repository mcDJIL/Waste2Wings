  ...
  return (
    <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4">
      {kpiData.map((kpi, i) => (
        <div
          key={i}
          className="flex flex-col justify-between gap-3 p-5 rounded-xl border-t-2 border border-[#C9A96E] bg-white/70 backdrop-blur-[10px] shadow-sm min-h-[140px] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md"
          ...
          <div>
            {kpi.showBar ? (
              <div className="flex justify-between items-end">
                ...
                <div className="w-16 h-2 rounded-full bg-[#BEC9C3] overflow-hidden mb-1">
                  <div className="h-full w-4/5 bg-[#006C49] rounded-full transition-all duration-700" />
                </div>
              </div>
            ) : (
              <p className={`text-xl font-bold leading-7 ${kpi.valueColor || 'text-[#051C37]'}`}>
            ...
            <p className={`text-sm font-semibold leading-6 mt-0.5 ${kpi.subColor}`}>{kpi.sub}</p>
          </div>
        </div>
      ))}
    </div>
